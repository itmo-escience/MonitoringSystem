...
public class InfrastructureMonitor {
    private static Logger log = LogManager.getLogger(InfrastructureMonitor.class);
    ...
    private String configFileName = "InfrastructureMonitor.config";
...
//               ApiServerUrl = "192.168.92.11:8081";
        InfrastructureMonitor infrastructureMonitor = new InfrastructureMonitor(new CommonMongoClient());
//        if(args!=null && args.length>0)
//            ApiServerUrl = args[0];
//        if (args!=null  && args.length>0 && args[0].equals("getData"))
//            infrastructureMonitor.GetMetricsFromDb("node-92-11", LocalDateTime.now().minusMinutes(1), LocalDateTime.now());
//        else
        ...
        infrastructureMonitor.StartMonitoring(sleepInterval);
    }

    public InfrastructureMonitor(CommonMongoClient mongoClient){
        readConfigFile();
